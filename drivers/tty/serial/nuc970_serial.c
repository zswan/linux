...
void nuc970serial_set_clock(struct device_node *node)
{
#if 0
	...
	clk = of_clk_get_by_name(node, "uart0");
	clk_prepare_enable(clk);

	clk = of_clk_get_by_name(node, "uart0_eclk");
	clk_prepare_enable(clk);
...
static int nuc970serial_probe(struct platform_device *pdev)
{
	...
	nuc970serial_set_clock(np);
	pdev->id = of_alias_get_id(np, "uart");
	if (pdev->id < 0)
		pdev->id = 0;
	...
	if (of_property_read_u32(np, "clock-frequency", &retval)) {
		dev_err(&pdev->dev, "clock-frequency property NOTset\n");
		return -EINVAL;
	}
	
	up->port.uartclk  = retval;
...
static struct of_device_id nuc970_uart_dt_ids[] = {
	{
		.compatible = "nuvoton,nuc970-uart",
	}, { /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, nuc970_uart_dt_ids);
...
static struct platform_driver nuc970serial_driver = {
	...
	.driver = {
		.name = "nuc970-uart",
		.of_match_table  = nuc970_uart_dt_ids,
	 },
};
